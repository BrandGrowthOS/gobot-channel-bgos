...
const BGOS_AGENT_HINTS_BODY = [
  ...
  "",
  "## Agent Boards (the [[BGOS_BOARDS]] marker round trip)",
  "Your owner keeps private tables (boards) in BGOS and shares them with",
  "chosen agents. On this channel you call the board operations by embedding",
  "one JSON object per marker block in a normal reply. The adapter strips",
  "every block from what the user sees, executes the calls as you, and sends",
  "the answers back to you as ONE follow up message in this same",
  "conversation. That answer message starts with the line",
  "[BGOS boards result] and is a system message from the adapter, NOT the",
  "user: read it, continue the task, do not thank anyone for it, and",
  "remember the user saw neither your request nor the raw result, so",
  "anything they should know must be said in a normal reply.",
  "Grammar, one call per block, up to 5 blocks per reply:",
  ...
  "- reqId is your correlation handle: any short string, unique per block.",
  "  Each result section is headed reqId=<yours> op=<op>, in the order you",
  "  asked.",
  "- op is one of: list, describe, create (name, optional description and",
  "  fields[{label,type}]), update_schema (action one of add_field,",
  "  update_field, delete_field, plus field and fieldKey as the action",
  "  needs), query (optional conditions, conjunction, sorts, search, limit,",
  "  cursor), get_row (row), insert (cells), update (row, cells), attach",
  "  (row, path to a local file on your host, optional name, mime,",
  "  fieldKey), search (query, optional limit), changes (optional since),",
  "  grant (assistantId, role).",
  "- board accepts a board id or its exact name. Row keys come from the key",
  "  column of query or search output; never invent or guess one.",
  '- Reads answer as markdown tables by default; add "format":"json" to a',
  "  read block for raw JSON.",
  "- A failed call answers as error status=<n> with the server's body",
  "  verbatim. Denials are deliberately uninformative about boards you",
  "  cannot see; do not probe around them. A malformed block answers",
  "  malformed_request with the grammar. After too many back to back board",
  "  result turns without a user message a loop_guard refusal pauses board",
  "  calls until real inbound arrives. Never claim a write happened when its",
  "  section shows an error.",
  "- Concurrency: call describe before writing to a board you have not used",
  "  this session, and search or query before inserting so the board keeps",
  "  one row per real thing. But query-then-insert is not atomic and update",
  "  is last write wins per cell with no notice to the loser, so on a board",
  "  with several writers confirm a claim or an insert with changes",
  "  afterward instead of trusting a clean read.",
  "",
  ...
  "    markdown, no `MEDIA:` lines, no buttons, no approval markers, no",
  "    `[[BGOS_BOARDS]]` blocks (a voice reply is captured raw, so a board",
  "    call there is spoken aloud and never executed),",
